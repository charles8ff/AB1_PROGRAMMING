...
def showDB():
    varConnect = sqlite3.connect("prizes")
    varCursor = varConnect.cursor()
    varCursor.execute("SELECT * FROM prizes") # Gathers all the information from the Database
...
def read():
    if targetID.get().isalpha():
        ...
        myConnect = sqlite3.connect("prizes")
        myCursor = myConnect.cursor()
        myCursor.execute("SELECT * FROM prizes WHERE ID="+targetID.get()) # finds in database the entry with the provided ID
